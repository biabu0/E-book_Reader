...
int main(int argc, char ** argv){
	...
	iError = DebugInit();
	if(iError){
		DBG_PRINTF(APP_ERR"DebugInit failed\n");
		return -1;
	}
	...
	while((iError = getopt(argc, argv, "ls:f:h:d")) != -1){
		switch(iError){
			...
			default:{
				DebugPrint(APP_INFO"Usage: %s [-s Size] [-f freetype_font_file] [-h HZK] <text_file>\n", argv[0]);
				DebugPrint(APP_INFO"Usage : %s -l\n", argv[0]);
				//	 结束运行即可
				return -1;
				break;
			}
	...
	iError = DrawInit();
	if(iError != 0){
		DebugPrint(APP_ERR"DisplayInit failed!\n");
		return -1;
	}
	// 启动使用Debug_print，对channel进行初始化


	// framebuffer 是 Linux 内核为显示设备提供的‌软件驱动接口‌，属于字符设备类别
	// 显示支持的类型
	if(bList){
		DebugPrint(APP_INFO"Support display: \n");
		ShowDispOpr();

		DebugPrint(APP_INFO"Support font: \n");
		ShowFontOpr();

		DebugPrint(APP_INFO"Support encoding: \n");
		ShowEncodingOpr();

		DebugPrint(APP_INFO"Support input: \n");
		ShowInputOpr();
		
		DebugPrint(APP_INFO"Support Debug channel: \n");
		ShowDebugOpr();
	...
	if(iError){
		DebugPrint(APP_ERR"OpenTextFile failed\n");
		return -1;
	}
	...
	if(iError){
		DebugPrint(APP_ERR"SetTextDetail failed\n");
		return -1;
	}
	...
	if (iError)
	{
		DebugPrint(APP_ERR"SelectAndInitDisplay error!\n");
		return -1;
	}
	...
	if (iError)
	{
		DebugPrint(APP_ERR"AllInputDeviceInit error!\n");
		return -1;
	}
	// 显示第一页
	DBG_PRINTF(APP_DEBUG"%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
	...
	if (iError)
	{
		DebugPrint(APP_ERR"Error to show first page\n");
		return -1;
	}
	DebugPrint(APP_INFO"Enter 'n' to show next page, 'u' to show previous page, 'q' to exit: ");
	DebugPrint(APP_INFO"Touch the upper of touchscreen to show previous page");
	DebugPrint(APP_INFO"Touch the lower of touchscreen to show next page");
	DebugPrint(APP_INFO"Touch the middle of touchscreen to quit");
